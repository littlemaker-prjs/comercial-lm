    ...
    <div className="flex h-screen bg-slate-50 font-sans text-slate-900 overflow-hidden relative">
      {notification && (
          <div className="absolute top-4 left-1/2 -translate-x-1/2 z-[100] animate-fade-in">
              <div className={`flex items-center gap-3 px-6 py-3 rounded-full shadow-xl border ${notification.type === 'success' ? 'bg-emerald-600 border-emerald-500 text-white' : 'bg-red-600 border-red-500 text-white'}`}>
      ...
      <aside className="w-64 bg-[#71477A] flex flex-col shadow-lg z-20 shrink-0">
        ...
        <div className="px-4 pt-4"><button onClick={backToDashboard} className="flex items-center gap-2 text-purple-200 hover:text-white text-xs transition-colors mb-2 w-full"><ArrowLeft className="w-3 h-3" />Dashboard</button></div>
        <nav className="flex-1 p-4 space-y-1">
          ...
          <button onClick={() => isClientConfigured && setCurrentStep(Step.WORKSHOPS)} disabled={!isClientConfigured} className={navItemClass(Step.WORKSHOPS, !isClientConfigured)}><LayoutDashboard className="w-5 h-5" /><span>Infra</span></button>
      ...
      <main className="flex-1 overflow-hidden relative">
        ...
        {currentStep === Step.PROPOSAL && <ProposalView appState={appState} setAppState={setAppState} onSave={handleSaveProposal} isSaving={isSaving} />}
      </main>
    </div>
  );
}
